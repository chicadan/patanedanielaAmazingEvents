let pastEvents = data.events.filter(elemento => new Date(elemento.date) < new Date(data.currentDate))
...
const checkjs = document. getElementById(checkGroupPast);

    let uniqueCat = []

    let categories = pastEvents.map (events => !uniqueCat.includes(events.category) ? uniqueCat.push(events.category): false);
...
let names = pastEvents.map (events => !eventsName.includes(events.name) ? eventsName.push(events.name): false);
...
//let past_events = data.events.filter(element => Date.dateReference < Date.referenceString)
